...
const GitHubAccessPage = () => {
	...
	const [lastSave, setLastSave] = useState<Dayjs | null>(
		dayjs(localStorage.getItem('facit_githubRequestTime')).isValid()
			? dayjs(localStorage.getItem('facit_githubRequestTime'))
			: null,
	);
		...
		onSubmit: (values) => {
			...
			fetch('/', {
				...
				.then(() => {
					...
					localStorage.setItem('facit_githubRequestTime', dayjs().format());
